...
Run the following command to check if a new version is available:

```bash
areawp update
```

If an update is available, the command will display the new version and prompt you to confirm before proceeding. No data is lost during updates.
