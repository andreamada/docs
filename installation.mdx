...
<AccordionGroup>
  <Accordion title="Set Up Wildcard SSL" icon="lock">
    **Enable HTTPS for All Subdomains**
    
    AreaWP creates client sites as subdomains, so a wildcard SSL certificate is highly recommended.
    ...
    **Option 1: Cloudflare API (Recommended)**
    ...
    3. In DirectAdmin, install Let's Encrypt wildcard certificate:
       ```bash
       # Via DirectAdmin SSL Manager
       # Select "Let's Encrypt" and enable wildcard
       ```
    
    **Option 2: Manual DNS Verification**
    
    1. Request Let's Encrypt certificate via DirectAdmin
    2. Add TXT record to DNS as prompted
    3. Verify and install certificate
    
    **Option 3: Commercial SSL**
    
    Purchase a wildcard SSL certificate (e.g., `*.yourdomain.com`) and install via DirectAdmin SSL Manager.
    
    <Tip>
    **Wildcard Format:** A wildcard SSL covers `*.yourdomain.com`, which includes `client1.yourdomain.com`, `client2.yourdomain.com`, etc.
    </Tip>
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Secure Admin Panel" icon="shield">
    **Protect Administrator Access**
    
    1. Change default admin password to a strong one
    2. Enable two-factor authentication (if available)
    3. Restrict admin IP addresses (optional)
    4. Set up SSL certificate for admin panel
